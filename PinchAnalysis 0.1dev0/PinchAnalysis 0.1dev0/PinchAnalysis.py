...
class pinchStream(object):
    ...
    def compositeCurve(Data='1', T_units='°C', Q_units='kW'):
        ...
        ax1.annotate(" "+str(max(heatFlowSink)-max(heatFlowSource))+" "+Q_units,
                     xy=(max(heatFlowSource), y_down),
                     xycoords='data',
                     xytext=(max(heatFlowSink), y_down),
                     arrowprops={'arrowstyle': '<->'},
                     textcoords='data')
        ax1.annotate(" " + str(min(heatFlowSink) - min(heatFlowSource)) + " " + Q_units,
                     xy=(min(heatFlowSource), y_up),
                     xycoords='data',
                     xytext=(min(heatFlowSink), y_up),
                     arrowprops={'arrowstyle': '<->'},
                     textcoords='data')
        return plt.show()
